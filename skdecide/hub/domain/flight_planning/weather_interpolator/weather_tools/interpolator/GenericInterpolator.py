...
from weather_interpolator.weather_tools.interpolator.WeatherInterpolator import WeatherInterpolator
...
def guess_axes(values: np.ndarray,
               lats: np.ndarray,
               longs: np.ndarray,
               levels: np.ndarray,
               times: np.ndarray):
    ...
    l = {"times": len(times), "levels": len(levels),
         "lats": len(lats), "longs": len(longs)}
...
class GenericEnsembleInterpolator(GenericInterpolator):
    ...
    """
    def __init__(self, file_npz, time_cut_index=None,
                 fields=None, time_shift_s=0.,
                 order=1):
        """
        ...
        if isinstance(file_npz, (str, np.lib.npyio.NpzFile)):
            self.datas = np.load(file_npz, allow_pickle=True) if isinstance(file_npz, str) else file_npz
            if fields is None:
            ...
            self.long_dict = {var: items[var]['longs']
                              if "longs" in items[var] else items[var]["lons"]
                              for var in items}
            self.levels_dict = {var: items[var]['levels']
                                if "levels" in items[var] else [200.]
                                for var in items}
            self.time_dict = {var: items[var]['times'] if self.time_cut_index is None
                              else items[var]['times'][:min(self.time_cut_index,
                                                            len(items[var]['times']))]
                              for var in items}
            if time_shift_s != 0.:
                for v in self.time_dict:
                    self.time_dict[v] += time_shift_s
            self.values = {var: items[var]['values']
                           for var in items}
        ...
        self.interpol_dict = {}
        for var in self.values:
            if len(self.levels_dict[var]) == 0 \
                    or (len(self.levels_dict[var]) == 1):
                self.levels_dict[var] = np.array([30000])
            # print(var, self.values[var].shape)
            axes = guess_axes(self.values[var],
                              self.lat_dict[var],
                              self.long_dict[var],
                              self.levels_dict[var],
                              self.time_dict[var])
            self.axes[var] = axes
            self.interpol_dict[var] = intergrid.Intergrid(self.values[var],
                                                          lo=[min(axes[1][i])
    ...
    def add_new_field(self, origin_field, operation, axis, new_field):
        ...
        axes = guess_axes(self.values[new_field],
        ...
        self.interpol_dict[new_field] = intergrid.Intergrid(self.values[new_field],
                                                            lo=[min(axes[1][i])
                                                                for i in range(len(axes[1]))],
                                                            hi=[max(axes[1][i])
                                                                for i in range(len(axes[1]))],
    ...
    def plot_field(self, field='issr',
                   alt=35000.,
                   t: Union[float, np.ndarray]=0.,
                   n_lat=180,
                   n_long=720,
                   ax=None):
        ...
        if self.values[field].ndim == 4:
            values = np.array([[time, alt,
                                range_lat[i],
                                range_long[j]] for i in range(n_lat) for j in range(n_long)
                               for time in times])
    ...
    def plot_field_5d(self, field='CONVECTION',
                      alt=35000.,
                      t: Union[float, np.ndarray]=0.,
                      n_lat=180,
                      index_forecast=0,
        ...
        for i in range(1, n_time):
            ...
            if save:
                ...
                t = "0" * (4 - len(str(j))) + str(j)
                plt.savefig(
                    os.path.join(folder, t + "_" + tag_file + ".png"))
            plt.pause(0.1)
        return m
    ...
    def render(self, ax, **kwargs):
        ...
        try:
            ax.imshow(self.values[dict_params["convexion_field"]][0, 0,
                                                                  dict_params.get("index_forecast", 0),
                                                                  :, :],
                      cmap='hot',
                      extent=[min(longs), max(longs),
                              min(lats), max(lats)],
...
class GenericWindInterpolator(GenericEnsembleInterpolator, WeatherInterpolator):
    ...
    def __init__(self, file_npz, time_cut_index=None, fields=None, order_interp=1, time_shift_s=0):
        """
        Stores the weather data and build the interpolators on grid.
        """
        super().__init__(file_npz, time_cut_index, fields=fields, time_shift_s=time_shift_s)
        if "U" not in self.values:
            ...
            ufield = "U"
            vfield = "V"
        self.norm_wind = np.sqrt(np.square(self.values[ufield]) + np.square(self.values[vfield]))
        self.angle_wind = np.arctan2(self.values[vfield], self.values[ufield])
        self.interpol_dict['norm-wind'] = intergrid.Intergrid(self.norm_wind,
                                                              lo=[min(self.axes[ufield][1][i])
                                                                  for i in range(len(self.axes[ufield][1]))],
                                                              hi=[max(self.axes[ufield][1][i])
                                                                  for i in range(len(self.axes[ufield][1]))],
                                                              maps=self.axes[ufield][1],
                                                              copy=True,
                                                              verbose=False,
                                                              order=1)
        self.interpol_dict['argument-wind'] = intergrid.Intergrid(self.angle_wind,
                                                                  lo=[min(self.axes[ufield][1][i])
                                                                      for i in range(len(self.axes[ufield][1]))],
                                                                  hi=[max(self.axes[ufield][1][i])
                                                                      for i in range(len(self.axes[ufield][1]))],
    ...
    def interpol_wind_classic(self, lat, longi,
                              ...
                              index_forecast=0):
        # with self._auto_lock:
        p = std_atm.alt2press(alt, alt_units='ft',
                              press_units='hpa')
    ...
    def interpol_wind(self, X):
        ...
        return self.interpol_dict['norm-wind'](X)*np.array([[np.cos(arg), np.sin(arg)]])
    ...
    def plot_wind(self, alt=35000.,
        ...
        x, y = XX, YY
        range_long = self.transform_long(range_long)
        values = np.array([[time, alt, index_forecast, range_lat[i], range_long[j]]
