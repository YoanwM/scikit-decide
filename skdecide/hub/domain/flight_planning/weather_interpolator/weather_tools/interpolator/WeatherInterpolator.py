...
import weather_interpolator.weather_tools.interpolator.intergrid as intergrid
import weather_interpolator.weather_tools.common_utils as Toolbox
import numpy as np
...
class WeatherForecastInterpolator(WeatherInterpolator):
    ...
    def __init__(self, file_npz, time_cut_index=None, order_interp=1):
        ...
        if isinstance(file_npz, (str, np.lib.npyio.NpzFile)):
            ...
            self.lat_dict = {var: self.datas[var].item()['lats'] for var in self.datas.keys()}
            self.long_dict = {var: self.datas[var].item()['longs'] for var in self.datas.keys()}
            self.alt_dict = {var: self.datas[var].item()['feets'] for var in self.datas.keys()}
            self.time_dict = {var: self.datas[var].item()['times'] if self.time_cut_index is None
                              else self.datas[var].item()['times'][:min(self.time_cut_index,
                                                                        len(self.datas[var].item()['times']))]
            ...
            if self.time_cut_index is not None:
                index_cut = min(self.time_cut_index, len(self.time_dict[list(self.time_dict.keys())[0]]))
            ...
            self.long_dict = {var: self.datas[var]['longs'] for var in self.datas.keys()}
            self.alt_dict = {var: self.datas[var]['feets'] for var in self.datas.keys()}
            self.time_dict = {var: self.datas[var]['times'] for var in self.datas.keys()}
            # Data Extraction
        ...
        self.interpol_dict = {}
        self.interpol_dict['wind'] = {'norm': intergrid.Intergrid(self.norm_wind,
                                      lo=[min(self.time_dict['UGRD']), min(self.alt_dict['UGRD']),
                                          min(self.lat_dict['UGRD']), min(self.long_dict['UGRD'])],
                                      hi=[max(self.time_dict['UGRD']), max(self.alt_dict['UGRD']),
                                          max(self.lat_dict['UGRD']), max(self.long_dict['UGRD'])],
                                      maps=[self.time_dict['UGRD'], self.alt_dict['UGRD'],
                                            self.lat_dict['UGRD'], self.long_dict['UGRD']],
                                      verbose=False, order=order_interp),
                                    'argument': intergrid.Intergrid(self.angle_wind,
                                                                    lo=[min(self.time_dict['UGRD']),
                                                                        min(self.alt_dict['UGRD']),
                                                                        min(self.lat_dict['UGRD']),
                                                                        min(self.long_dict['UGRD'])],
                                                                    hi=[max(self.time_dict['UGRD']),
                                                                        max(self.alt_dict['UGRD']),
                                                                        max(self.lat_dict['UGRD']),
                                                                        max(self.long_dict['UGRD'])],
                                                                    maps=[self.time_dict['UGRD'],
                                                                          self.alt_dict['UGRD'],
                                                                          self.lat_dict['UGRD'],
                                                                          self.long_dict['UGRD']],
                                                                    verbose=False, order=order_interp)
                                      }
        self.interpol_dict['humidity'] = intergrid.Intergrid(self.humidity,
                                                             lo=[min(self.time_dict['RH']), min(self.alt_dict['RH']),
                                                                 min(self.lat_dict['RH']), min(self.long_dict['RH'])],
                                                             hi=[max(self.time_dict['RH']), max(self.alt_dict['RH']),
                                                                 max(self.lat_dict['RH']), max(self.long_dict['RH'])],
                                                             maps=[self.time_dict['RH'], self.alt_dict['RH'],
                                                                   self.lat_dict['RH'], self.long_dict['RH']],
                                                             verbose=False, order=order_interp)
        self.interpol_dict['temperature'] = intergrid.Intergrid(self.temperature,
                                                                lo=[min(self.time_dict['TMP']),
                                                                    min(self.alt_dict['TMP']),
                                                                    min(self.lat_dict['TMP']),
                                                                    min(self.long_dict['TMP'])],
                                                                hi=[max(self.time_dict['TMP']),
                                                                    max(self.alt_dict['TMP']),
                                                                    max(self.lat_dict['TMP']), max(self.long_dict['TMP'])],
                                                                maps=[self.time_dict['TMP'], self.alt_dict['TMP'],
                                                                      self.lat_dict['TMP'], self.long_dict['TMP']],
                                                                verbose=False,
                                                                order=order_interp)

    def interpol_wind_classic(self, lat, longi,
                              alt=35000., t=0., **kwargs):
        """
    ...
    def interpol_wind(self, X):
        ...
        return self.interpol_dict['wind']['norm'](X)*np.array([[np.cos(arg), np.sin(arg)]])
    ...
    def plot_wind(self, alt=35000.,
                  down_long=-180., up_long=180.,
                  down_lat=-90., up_lat=90.,
                  t=0., n_lat=180,
                  n_long=720, plot_wind=False, ax=None):
        """
        ...
        XX, YY = np.meshgrid(range_long,
                             range_lat)
        x, y = XX, YY
        range_long = self.transform_long(range_long)
        values = np.array([[time, alt, range_lat[i], range_long[j]]
                           for i in range(n_lat) for j in range(n_long)
                           for time in times])
        ...
        if plot_wind:
            if x.shape[0] > 100:
                q = ax.quiver(x[::10, ::10], y[::10, ::10], Ut[::10, ::10, i], Vt[::10, ::10, i], alpha=0.8)
            else:
    ...
    def plot_wind_noised(self, alt=35000.,
                         down_long=-180., up_long=180.,
                         down_lat=-90., up_lat=90.,
                         t=0., n_lat=180,
                         n_long=720, plot_wind=False,
                         mean_noised_norm=1.05, scale_noised_norm=0.01,
                         mean_noised_arg=0.1, scale_noised_arg=0.01,
                         ax=None):
        """
        Plot the wind for a given coordinates window for a given altitude and for one time/or range of time
        ...
        noised_norm = np.random.normal(mean_noised_norm, scale=scale_noised_norm, size=norm.shape)
        norm = noised_norm*norm
        noised_arg = np.random.normal(mean_noised_arg, scale=scale_noised_arg, size=arg.shape)

        res_noised = norm * np.array([[np.cos(arg+noised_arg), np.sin(arg+noised_arg)]])
        res = self.interpol_wind(values)
        ...
        if plot_wind:
            if x.shape[0] > 100:
                q = ax.quiver(x[::10, ::10], y[::10, ::10],
                              Ut_noised[::10, ::10, i],
                              Vt_noised[::10, ::10, i], alpha=0.8)
            else:
                q = ax.quiver(x, y, Ut_noised[:, :, i]-Ut[:, :, i],
                              Vt_noised[:, :, i]-Vt[:, :, i], alpha=0.8)
        plt.title('time : ' + str(times[i]))
        for i in range(1, n_time):
            for coll in CS.collections:
                plt.gca().collections.remove(coll)
            ...
            if plot_wind:
                ...
                if x.shape[0] > 100:
                    q = ax.quiver(x[::10, ::10], y[::10, ::10],
                                  Ut_noised[::10, ::10, i], Vt_noised[::10, ::10, i], alpha=0.8)
                else:
                    q = ax.quiver(x, y, Ut_noised[:, :, i], Vt_noised[:, :, i], alpha=0.8)
            plt.title('time : ' + str(times[i]))
            plt.draw()
            plt.pause(0.1)
        return ax
    ...
    def plot_matrix_wind(self, index_alt=10, index_time=0, ax=None):
        ...
        XX, YY = np.meshgrid(range_long,
                             range_lat)
        x, y = XX, YY
        norm = np.sqrt(np.square(self.temperature[index_time, index_alt, :, :])
                       + np.square(self.temperature[index_time, index_alt, :, :]))
        ax.imshow(norm, cmap='hot', interpolation='nearest',
                  extent=[-180., 180.,
                          -90., 90.],
                  alpha=0.5)
    ...
    def plot_matrix_wind_noised(self, index_alt=10, index_time=0, ax=None):
        ...
        XX, YY = np.meshgrid(range_long,
                             range_lat)
        x, y = XX, YY
        norm = np.sqrt(np.square(self.u_wind[index_time, index_alt, ::-1, :])
                       + np.square(self.v_wind[index_time, index_alt, ::-1, :]))
        ax.imshow(norm, cmap='hot', interpolation='nearest', extent=[-180., 180., 90., -90.], alpha=0.5)
        plt.gca().invert_yaxis()
        ax.colorbar()

    def plot_field(self, field='issr', alt=35000., t=0., n_lat=180, n_long=720, ax=None):
        """
        ...
        values = np.array([[time, alt, range_lat[i], range_long[j]] for i in range(0, n_lat) for j in range(0, n_long)
                           for time in times])
        Ut = np.resize(self.interpol_dict[field](values), (n_lat, n_long, n_time))
        i=0
        cs = ax.contour(x, y, Ut[:, :, i],
